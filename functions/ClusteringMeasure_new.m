...
[mat,~] = confusionmat(Y,res);
matsum_column = sum(mat);
matsum_row = sum(mat,2);
for iter = 1:pred_classnum
    precision(iter) = mat(iter,iter)/(matsum_column(iter));
    recall(iter) = mat(iter,iter)/(matsum_row(iter));
end
